...
import sys
sys.path.append('C:\\Users\\Tony\\Desktop\\srdiff_true') # 改
from utils_self.hparams import hparams, set_hparams
import numpy as np
from utils_self.utils import plot_img, move_to_cuda, load_checkpoint, save_checkpoint, tensors_to_scalars, load_ckpt, Measure
...
class Trainer:
    ...
    def train(self):
        ...
        for batch in train_pbar:
            ...
            print(len(train_pbar))    
            print(training_step)
            
            if training_step % 480 == 0:    
    ...
    def test(self):
        ...
        if hparams['test_save_png']:
            ...
            os.makedirs(os.path.join(self.gen_dir, 'outputs'), exist_ok=True)
            os.makedirs(os.path.join(self.gen_dir, 'SR'), exist_ok=True)
...
# ------vscode
# python tasks/trainer.py --config configs/rrdb/df2k4x_pretrain.yaml --exp_name EEGAN_train_274_64_64_94804_difficult_48994 --reset


# -----------------train srdiff-----------------------------------
# -------vscode
# python tasks/trainer.py --config configs/diffsr_df2k4x.yaml --exp_name diffsr_div2k_2_try --reset --hparams="rrdb_ckpt=checkpoints/rrdb_div2k_2"


# ----------------tensorboard---------------------------------------
# -------vscode
# tensorboard --logdir checkpoints/diffsr_div2k_2_try



# ----------------evaluate------------------------------------------
# -------vscode
# python tasks/trainer.py --config configs/diffsr_df2k4x.yaml --exp_name diffsr_div2k_ssim_EEGAN_train_274_64_64_45902_difficult_23713_second --infer
