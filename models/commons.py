...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        # Create interleaved sin and cos embeddings
        emb_sin = emb.sin()
        emb_cos = emb.cos()
        interleaved_emb = torch.empty((emb_sin.size(0), emb_sin.size(1) * 2), device=device)
        interleaved_emb[:, 0::2] = emb_sin
        interleaved_emb[:, 1::2] = emb_cos
