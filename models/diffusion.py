...
from utils_self.sr_utils import SSIM, PerceptualLoss
from utils_self.hparams import hparams
...
class GaussianDiffusion(nn.Module):
    ...
    def ddim_sample(self, x, t, cond, img_lr_up, eta=0.0):
        b, *_, device = *x.shape, x.device
        alpha_cumprod_t = extract(self.alphas_cumprod, t, x.shape)
        alpha_cumprod_t_prev = extract(self.alphas_cumprod_prev, t, x.shape)

        sqrt_one_minus_alpha_cumprod_t = extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape)
        sqrt_recip_alpha_cumprod_t = extract(self.sqrt_recip_alphas_cumprod, t, x.shape)
        sqrt_recipm1_alpha_cumprod_t = extract(self.sqrt_recipm1_alphas_cumprod, t, x.shape)

        # Predict x0
        noise_pred = self.denoise_fn(x, t, cond=cond, img_lr_up=img_lr_up)
        x0_pred = (x - sqrt_one_minus_alpha_cumprod_t * noise_pred) / sqrt_recip_alpha_cumprod_t

        # DDIM sampling
        if t == 0:
            return x0_pred, x0_pred

        sigma = eta * (1 - alpha_cumprod_t_prev) / (1 - alpha_cumprod_t)
        sigma = torch.sqrt(sigma)
        noise = noise_like(x.shape, device, repeat=False)
        x_prev = (
            sqrt_recip_alpha_cumprod_t * (x - sqrt_one_minus_alpha_cumprod_t * noise_pred) +
            sigma * noise
        )
    ...
    @torch.no_grad()
    def p_sample(self, x, t, cond, img_lr_up, noise_pred=None, clip_denoised=True, repeat_noise=False, use_ddim=False, eta=0.0):
        if use_ddim:
            return self.ddim_sample(x, t, cond, img_lr_up, eta)
    ...
    @torch.no_grad()
    def sample(self, img_lr, img_lr_up, shape, save_intermediate=False, use_ddim=False, eta=0.0):
        ...
        for i in it:
            img, x_recon = self.p_sample(
                img, torch.full((b,), i, device=device, dtype=torch.long), cond, img_lr_up, use_ddim=use_ddim, eta=eta)
    ...
    @torch.no_grad()
    def interpolate(self, x1, x2, img_lr, img_lr_up, t=None, lam=0.5, use_ddim=False, eta=0.0):
